...
void conjugate_gradients(LocalData* local, int N, int max_iterations) {
    ...
    if (!r) {
        printf("Process %d: failed to allocate r vector\n", rank);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    
    double* z = malloc(N * sizeof(double));
    if (!z) {
        printf("Process %d: failed to allocate z vector\n", rank);
        free(r);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    if (!local_part) {
        printf("Process %d: failed to allocate local_part\n", rank);
        free(r);
        free(z);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    if (!full_part) {
        printf("Process %d: failed to allocate full_part\n", rank);
        free(r);
        free(z);
        free(local_part);
        MPI_Abort(MPI_COMM_WORLD, 1);
