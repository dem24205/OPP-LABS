...
void conjugate_gradients(LocalData* local, int N, int max_iterations) {
    ...
    double* local_r = malloc(local->local_rows * sizeof(double)); 
    if (!local_r) {
    ...
    double* full_z = malloc(N * sizeof(double)); 
    if (!full_z) {
        free(local_Az);
        free(local_r);
        free(local_z);
    ...
    // Сборка вектора x0 со всех процессов
    MPI_Allgatherv(local->local_x,
        local->local_rows,
        MPI_DOUBLE,
        full_z, 
        local->send_counts, 
    ...
    //Az = A * x0
    matvec_local(local, full_z, local_Az, N);
    ...
    double r_norm;
    do {
        MPI_Allgatherv(local_z, local->local_rows, MPI_DOUBLE,
                       full_z, local->send_counts, local->displacements,
