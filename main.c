...
bool allocate_local_blocks(double **Ablock, double **Bblock, double **Cblock, 
                           int rows, int n2, int cols) {
    *Ablock = calloc(rows * n2, sizeof(double));
    if (!*Ablock) {
        return false;
    }
    
    *Bblock = calloc(n2 * cols, sizeof(double));
    if (!*Bblock) {
        free(*Ablock);
        return false;
    }
...
int main(int argc, char **argv) {
    ...
    //Подбор размеров двумерной решетки под количество процессов
    int dims[2] = {0, 0};   
    MPI_Dims_create(size, 2, dims);
    int px = dims[0], py = dims[1];

    //Создание коммуникатора grid_comm, в котором процессы организованы в решетку
    int periods[2] = {0, 0};
    MPI_Comm grid_comm;
    MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periods, 0, &grid_comm);

    //Преобразование ранга процесса в координаты решетки
    int coords[2];
    ...
    //разрезание решетки на подрешетки
    MPI_Comm row_comm, col_comm;
    int remain_dims[2];
    remain_dims[0] = 0; //процессы с разными значениями строк решетки - в разных группах
    remain_dims[1] = 1; 
    MPI_Cart_sub(grid_comm, remain_dims, &row_comm);
    remain_dims[0] = 1; 
    remain_dims[1] = 0; //процессы с разными значениями столбцов решетки - в разных группах 
    ...
    //все процессы в первом столбце решетки получают части матрицы А
    if (coords[1] == 0) {
    ...
    //все процессы в первой строке решетки получают части матрицы В
    if (coords[0] == 0) {
    ...
    if (rank == 0) {
        ...
        //вычисление смещений для сбора матрицы
